def run_registration_debug_deform(fixed_wb_ct, warped_ob_ct, transforms, output_dir):
    ...
    jac_penalty = 0
    try:
        jac = ants.create_jacobian_determinant_image(fixed_wb_ct, transforms[0]).numpy()
...
def register_patient_data_deform(wb_dir, ob_dir, output_dir, debug=False):
    ...
    ants.image_write(warped, str(output_dir / "Warped_OB_CT_Deform.nii.gz"))

    score = None
    if debug:
        score = run_registration_debug_deform(fixed, warped, reg['fwdtransforms'], output_dir)
