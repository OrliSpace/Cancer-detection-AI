...
from lib.transforms.multimodal import AddPETPathd
...
class Segmentation(BasicTrainTask):
    ...
    def loss_function(self, context: Context):
        return DiceCELoss(to_onehot_y=True, softmax=True, include_background=False, squared_pred=True)
    ...
    def train_pre_transforms(self, context: Context):
        return [
            AddPETPathd(keys=["image"]),
            LoadImaged(keys=("image", "pet_image", "label")),
            NormalizeLabelsInDatasetd(keys="label", label_names=self._labels),  # Specially for missing labels
            EnsureChannelFirstd(keys=("image", "pet_image", "label")),
            EnsureTyped(keys=("image", "pet_image", "label"), device=context.device),
            Orientationd(keys=("image", "pet_image", "label"), axcodes="RAS"),
            Spacingd(keys=("image", "pet_image", "label"), pixdim=self.target_spacing, mode=("bilinear", "bilinear", "nearest")),
            ScaleIntensityRanged(keys="image", a_min=-160, a_max=240, b_min=0.0, b_max=1.0, clip=True),
            NormalizeIntensityd(keys="pet_image", nonzero=True),
            ConcatItemsd(keys=["image", "pet_image"], name="image", dim=0),
            CropForegroundd(
                keys=("image", "label"),
                source_key="image",
                margin=10,
    ...
    def val_pre_transforms(self, context: Context):
        return [
            ...
            LoadImaged(keys=("image", "pet_image", "label")),
            NormalizeLabelsInDatasetd(keys="label", label_names=self._labels),  # Specially for missing labels
            EnsureTyped(keys=("image", "pet_image", "label")),
            EnsureChannelFirstd(keys=("image", "pet_image", "label")),
            Orientationd(keys=("image", "pet_image", "label"), axcodes="RAS"),
            Spacingd(keys=("image", "pet_image", "label"), pixdim=self.target_spacing, mode=("bilinear", "bilinear", "nearest")),
            ScaleIntensityRanged(keys="image", a_min=-160, a_max=240, b_min=0.0, b_max=1.0, clip=True),
